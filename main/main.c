...
void app_main(void)
{
    ...
    LOG_DEBUG(TAG, "This is %s chip with %d CPU core(s), %s%s%s%s, ",
           CONFIG_IDF_TARGET,
           chip_info.cores,
    ...
    LOG_DEBUG(TAG, "silicon revision v%d.%d, ", major_rev, minor_rev);
    if(esp_flash_get_size(NULL, &flash_size) != ESP_OK) {
        LOG_ERROR(TAG, "Get flash size failed");
        return;
    }

    LOG_DEBUG(TAG, "%" PRIu32 "MB %s flash", flash_size / (uint32_t)(1024 * 1024),
